...
| 多榫卯方案 | 一个方案可挂多个榫卯（如面板拼板 + 框架直榫 + 抽屉燕尾），每个榫卯选择作用的两个零件；零件可增删改名、填长/宽/厚与数量，截面尺寸与榫卯参数双向同步 |
| 三视图出图 | 正视图 / 俯视图 / 侧视图共享同一套几何函数生成，保证宽度一致；自动尺寸标注、齿序编号；编辑器左侧逐个切换榫卯，中间只画当前榫卯 |
| 锯路补偿 | 图纸同时标注理论线与锯切线（向废料侧偏移 kerf/2），切割步骤按"先锯废料侧"排序；右侧切割步骤按榫卯分组 |
| 配合余量表 | 硬木/软木 × 紧/标准/松 六格经验值，可编辑并持久化 |
| 方案库 | localStorage 保存方案，按类型/板厚筛选（多榫卯方案任一命中即匹配），支持 JSON 导出/导入（往返完全一致，旧版单榫卯数据自动迁移） |
| 打印 1:1 | A4 打印视图内置 100mm 校验尺，打印后实测可验证缩放；按榫卯逐段出图，各段自带齿号表与切割步骤，齿形模板页可直接描线 |
...
| 层级 | 数量 | 覆盖点 |
| --- | --- | --- |
| 单元（vitest） | 56 | 随机 200 组燕尾闭合 ≤ 0.1mm（mulberry32 固定种子）、直榫 10 组手工核算、三视图一致性、边界警告、锯切线数量、导出导入往返全等、多榫卯关联/同步/迁移、性能 |
| 组件（RTL） | 16 | 新建流程、键盘 ±0.5 微调、脏参数提示条、类型切换联动、列表筛选、非法导入错误提示、表单钳制、多榫卯切换/挂零件/零件清单/分组步骤/多段打印 |
| E2E（Playwright） | 8 | 全流程（新建→出图→保存→列表）、警告场景、键盘微调、导出→删除→导入 viewBox 一致、打印校验尺、五类型出图、多榫卯方案（拼板+直榫+燕尾）、知识卡 |
