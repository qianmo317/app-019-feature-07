...
import { defaultParams, makePlan, upsertPlan } from '../store/plans'
import { navigate } from '../router'
import { KindPicker, ParamForm } from '../components/ParamForm'

export function NewPlanPage() {
  const [kind, setKind] = useState<JointKind | null>(null)
  const [params, setParams] = useState<Params>(() => defaultParams('dovetail'))

  const selectKind = (k: JointKind) => {
    setKind(k)
    setParams((prev) => ({ ...defaultParams(k), boardA: prev.boardA, boardB: prev.boardB, wood: prev.wood, fit: prev.fit, kerfMm: prev.kerfMm }))
  }
  ...
  return (
    <div className="page" data-testid="new-page">
      <h1>新建方案</h1>
      <h2>1. 选择榫卯类型</h2>
      <KindPicker value={kind} onChange={selectKind} />
      <h2>2. 填写参数</h2>
      {kind ? (
